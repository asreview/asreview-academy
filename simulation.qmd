...
A simulation in the context of AI-assisted reviewing involves mimicking the screening process with a particular model. In a simulation setting, it is already known which records are labeled as relevant, and the software can automatically reenact the screening process as if a human were labeling the records in interaction with the Active Learning model. This allows you to answer questions about the (workings of) the model and compare the performance of different models under different circumstances. Read the [documentation](https://asreview.readthedocs.io/en/stable/lab/simulation_overview.html){target="_blank"} for more information about running simulations.
...
To start off easy, we will run a simulation study using the `van de Schoot et al. (2018)` benchmark dataset we also used in the [introductory exercise](ASReviewLAB.qmd){target="_blank"}.
...
![](images/Simulation/v2_simulation_mode.png){width=85% fig-align="center"}
...
![](images/Simulation/v2_start_project.png){width=85% fig-align="center"}
...
![](images/Simulation/v2_simulation_finished.png){width=85% fig-align="center"}
...
![](images/Simulation/v2_project_finished.png){width=85% fig-align="center"}

Note: it can be that running the simulation takes a long time because of the size of your dataset in combination with the chosen model. To give you an idea of the possible differences in runtime between models, see Table 1 on page 15 of [Teijema et al. (2023)](https://doi.org/10.3389/frma.2023.1178181){target="_blank"}. Performing the feature matrix for a dataset with 15K records takes 20 seconds with TF-IDF, and 6 hours with SBERT.


![](images/Simulation/v2_analytics.png){width=85% fig-align="center"}
...
> Have you read the `AI Models Explained` section revealed by clicking the lightbulb icon? If not, you can do so now to learn more about the different model types.

![](images/Simulation/v2_multiple_projects.png){width=85% fig-align="center"}
...
Many more options for the configuration are available via the Command Line Interface (CLI). Read the documentation on the [CLI](https://asreview.readthedocs.io/en/stable/lab/simulation_cli.html){target="_blank"} to get started. 
...
3.  Does, for example, the ELAS u4 or ELAS u3 perform better for a given dataset, when keeping the other settings fixed? For this question, we introduce the metrics the Extra Relevant Records found (ERF) and the Average Time to Discovery (ATD).
...
``` bash 
asreview simulate data/[name_of_your_data.csv] --output output/[results_name].asreview --seed [your favorite number]
```

Save this script in the `jobs.txt` file you just made, to keep track of which code you ran!

You can also decide to [supply more arguments](https://asreview.readthedocs.io/en/stable/lab/simulation_cli.html){target="_blank"}, for example to employ a different model or to specify prior knowledge yourself instead of using `--prior_seed` to select random ones . If no model is specified, the software’s [default active learning model](https://asreview.readthedocs.io/en/stable/lab/models.html){target="_blank"} is used.
...
``` bash 
pip install asreview-insights asreview-makita
```
...
To answer <u>research question 2</u>, you can rerun the same model once more with a different value for `--prior_seed`. A different seed value will result in a different selection of prior knowledge.
...
``` bash 
asreview simulate data/[name_of_your_data.csv] --state_file output/[results_name].asreview --seed [your favorite number] --prior_idx [row nr. relevant record] [row nr. irrelevant record] 
```
...
To answer <u>research question 3</u>, rerun the model with the `ELAS u3` model  instead of the default setting:

``` bash 
asreview simulate data/[name_of_your_data].csv --ai elas_u3 --output output/[results_name].asreview  --seed [your favorite number] --prior-seed [your second favorite number] 
```
...
Read the [Getting Started documentation](https://github.com/asreview/asreview-makita?tab=readme-ov-file#getting-started){target="_blank"} to get started with Makita. Follow the steps below to get familiar with the basic functionality of Makita.

Start with creating a new project folder. You can do this in the same way as you did for the CLI exercise. Inside the project folder, create a subfolder called `data` and put at least two [benchmark datasets](https://github.com/asreview/synergy-dataset#datasets){target="_blank"} in the 'data' folder. You can copy files from the previous exercise, or use your own datasets.
...
![](images/Simulation/v2_makita.png){width=85% fig-align="center"}
